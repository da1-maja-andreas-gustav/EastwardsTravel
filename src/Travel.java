...
public class Travel {
    public int shortestPath(int[] nodeWeights, Edge[][] edgeLists) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < edgeLists[i].length; j++) {
                Edge edge = edgeLists[i][j];
                bestCost[edge.to] = relax(edge.from,edge.to,nodeWeights,edge.weight,bestCost);
            }
        }
    ...
    private int relax(int from, int to,int[] nodeWeights, int edgeWeight, int[] bestCost) {
        int toWeight = nodeWeights[to];
        if (bestCost[to] > edgeWeight + bestCost[from] + toWeight) {
            return edgeWeight + bestCost[from] + toWeight;
        }
